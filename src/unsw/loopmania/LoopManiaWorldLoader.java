...
public abstract class LoopManiaWorldLoader {
    ...
    public LoopManiaWorld load() {
        ...
        for (int i = 0; i < jsonRareItems.length(); i++) {
            world.loadRareItems(jsonRareItems.getString(i));
        }
        
        JSONObject currentJson = json.getJSONObject("goal-condition");
        world.loadGoals(currentJson);
        // loadGoals(world, json.getJSONObject("goal-condition"));
